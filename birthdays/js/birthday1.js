...
{ name: '沢渡 真琴（《Kanon》）', birthday: '01-06' },
    ...
    // 过滤出当天及未来一个月内过生日的角色
    const upcomingBirthdays = storedBirthdays.filter(item => {
    let [month, day] = item.birthday.split('-').map(Number); // 分割月份和日期
    let birthday = new Date(today.getFullYear(), month - 1, day); // 注意月份是从0开始的

    if (birthday < today) { // 如果生日已经过去，则设置为下一年
    birthday.setFullYear(today.getFullYear() + 1);
}

    // 检查当天及未来一个月内的生日
    return (birthday - today) <= 1000 * 60 * 60 * 24 * 30;
});
    ...
    let daysUntilBirthday = Math.ceil((new Date(today.getFullYear(), birthday.birthday.split('-')[0] - 1, birthday.birthday.split('-')[1]) - today) / (1000 * 60 * 60 * 24));

    if (daysUntilBirthday === 0) {
    listItem.textContent = `${birthday.name} 今天过生日！`;
} else {
    listItem.textContent = `${birthday.name} 生日: ${birthday.birthday}，(还有 ${daysUntilBirthday} 天)`;
}

    listElement.appendChild(listItem);
});

    // 如果未来一个月内没有人生日，则显示提示信息
    if (upcomingBirthdays.length === 0) {
    const noBirthdaySoon = document.createElement('p');
    noBirthdaySoon.textContent = '未来一个月内没有人过生日。';
    listElement.appendChild(noBirthdaySoon);
}
});
